...
-- The `::text::jsonb` cast is load-bearing: it makes sqlc type the param as a Go string (from the
-- inner ::text) so pgx sends it as text under QueryExecModeExec (the Neon pooler mode), which
-- Postgres coerces to jsonb. A raw []byte param would be sent as bytea, which jsonb rejects.
UPDATE users
SET "editorPreferences" = sqlc.arg('editor_preferences')::text::jsonb, "updatedAt" = now()
...
-- Per-day item-creation counts for the mission-control heatmap window. Groups the user's
-- items by their UTC calendar creation date, from window_start (todayUTC - 83 days, passed
-- by the handler) forward. Mirrors the legacy getDashboardActivity raw query: the handler
-- fills the missing days (count 0) and buckets each day's level 0–4, so this query returns
-- only the days that actually have items. Scoped to the session user (IDOR-safe).
SELECT ("createdAt" AT TIME ZONE 'UTC')::date AS day, COUNT(*)::int AS count
FROM items
WHERE "userId" = sqlc.arg('owner')
  AND ("createdAt" AT TIME ZONE 'UTC')::date >= sqlc.arg('window_start')::date
GROUP BY day;
