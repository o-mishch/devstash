...
type Store interface {
	...
	GetDashboardActivity(
		ctx context.Context, arg sqlcdb.GetDashboardActivityParams,
...
type Deps struct {
	...
	Logger   *slog.Logger
	// Now anchors the dashboard-activity window to one clock (time.Now in production, a
	// fixed instant in tests). Injected as a seam so the 84-day series is deterministic.
	Now func() time.Time
}
