...
| R8 | CI deploy fails at the **first `helm`/`kubectl` step** with `Error: Kubernetes cluster unreachable: <html> … Error 403 (Forbidden)!!1 … That's an error`, while the `get-gke-credentials` step stays **green** | The generic **Google HTML 403** page is the DNS endpoint's **Google Front End rejecting the GitHub runner's EXTERNAL traffic at the network layer** — before IAM or kube-apiserver. It is **not** an IAM denial (that names the permission: `Permission 'container.clusters.connect' denied on resource …`) and **not** the `roles/container.developer` IAM condition (that condition already passes `container.clusters.get` at the creds step, and `container.googleapis.com/Cluster` is a [resource type that supports `resource.name` conditions](https://cloud.google.com/iam/docs/conditions-resource-attributes)). The creds step succeeds because it reads the cluster over the always-on regional `container.googleapis.com` API, **not** the DNS endpoint. The live cluster has `allow_external_traffic` effectively **off** — drift from the Terraform, which already declares `allow_external_traffic = true` in `modules/gke/main.tf` (`control_plane_endpoints_config.dns_endpoint_config`). | **No code value changed — the config was already correct.** Reconcile the drift (non-destructive): `tofu apply`, or hotfix `gcloud container clusters update devstash-dev-gke --region us-central1 --enable-dns-access`. Confirm with `gcloud container clusters describe devstash-dev-gke --region us-central1 --format='value(controlPlaneEndpointsConfig.dnsEndpointConfig.allowExternalTraffic)'` (expect `True`). Added drift-guard comments to `modules/gke/main.tf` and `modules/iam/main.tf`, plus a fail-fast **`Verify control plane reachable (DNS endpoint)`** preflight in `deploy-gke.yml` that detects the 403 signature and prints the exact remediation. **Do NOT** flip `allow_external_traffic` to false or remove the IAM condition chasing this symptom. |
