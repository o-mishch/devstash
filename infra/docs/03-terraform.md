...
| Upstash Redis (REST API) | Memorystore | Без зміни коду: app ходить нативно по TCP через `node-redis` прямо в Memorystore, увімкнено `REDIS_URL` (`src/lib/infra/redis-tcp.ts`). На Vercel `REDIS_URL` не задано → лишається `@upstash/redis` REST. |
