...
> [`devstash-infra local`](../cli/README.md) (див. TL;DR). 📚-концепти
> Kubernetes — у [02-kubernetes.md](02-kubernetes.md); тут — реальний запуск наживо.

## Підсумок (TL;DR)

```bash
devstash-infra local up              # підняти все (= up)
curl 'http://localhost:8080/api/health?deep=1'   # → {db,redis,s3,email all "ok"}
devstash-infra local deploy   # перезібрати образ і викотити лише застосунок
devstash-infra local status   # стан кластера / подів / health
devstash-infra local info     # вивести всі URL сервісів (застосунок, Postgres, MinIO тощо)
devstash-infra local down     # знести кластер
```

Застосунок піднімається з тих самих базових маніфестів, що й GCP
(`infra/k8s/base` через `infra/k8s/overlays/local/kustomization.yaml`) — тобто локально
...
Саме ці значення використовує Secret/ConfigMap застосунку `devstash-secrets`/
`devstash-config` (так само названі, як у базі та GCP-оверлеї — локальний запуск
тепер збирається з `infra/k8s/base` через `infra/k8s/overlays/local/kustomization.yaml`):

| Змінна | Джерело | Значення |
|--------|---------|----------|
| `DATABASE_URL` | Secret | `postgresql://devstash:devstash@postgres:5432/devstash?sslmode=disable` |
| `REDIS_URL` | Secret | `redis://redis:6379` (нативний node-redis, як у GCP) |
| `AWS_ENDPOINT_URL_S3` | ConfigMap | `http://localhost:9000` (не `minio:9000` — див. «Завантаження файлів» нижче) |
| `SMTP_HOST` / `SMTP_PORT` | ConfigMap | `mailpit` / `1025` |
...
```bash
# Уже піднято devstash-infra local. Відкрий:
...
Маніфест — `infra/k8s/local/06-headlamp.yaml`.
...
| Redis у кластері (нативний node-redis) | ✅ `redis-stack` (latest) — той самий node-redis-шлях, що й GCP→Memorystore |
...
`*-local.ts`, що активуються наявністю connection-конфігу: `DB_DRIVER` (один явний
прапорець — `DATABASE_URL` не відрізняє Neon від Cloud SQL), `REDIS_URL`,
`AWS_ENDPOINT_URL_S3`, `SMTP_HOST` (їх задає локальний ConfigMap — не Secret, обидва
не секретні; на GCP SMTP_HOST відсутній → Resend). На Vercel їх немає → код no-op,
прод незмінний.
...
style, без sidecar. Усі три зміни — лише в `infra/k8s/overlays/local/` + локальному ConfigMap.
...
- **Найменш інвазивно:** вибір адаптера винесено в `src/lib/infra/db-local.ts`.
  ...
  Локальний адаптер вмикається лише за `DB_DRIVER=pg` (його задають локальний
  Secret/ConfigMap і GCP overlay — Cloud SQL теж іде через node-postgres);
  у проді (Vercel) → `null` → реальний Neon-адаптер незмінний.
...
(`DB_DRIVER` / `REDIS_URL` / `AWS_ENDPOINT_URL_S3` / `SMTP_HOST`). На Vercel/проді цих змінних
...
### Redis — нативний node-redis (як у GCP)
...
він натомість підключається **нативно по TCP через node-redis** прямо до Redis —
**без SRH-проксі**. Перемикач — змінна `REDIS_URL`:

- `REDIS_URL` задано (GKE→Memorystore, локальний kind) → `getRedis()` повертає
  node-redis-адаптер з тим самим інтерфейсом, що й Upstash-клієнт
  ...
  **не змінюються**. Rate-limiting переходить на sliding-window Lua-скрипт
  (`@upstash/ratelimit` не підтримує node-redis).
...
```
infra/local/                # сайдкари локального оркестратора (сам оркестратор — у devstash-infra CLI)
├── valkey-openssl.cnf      # SAN-конфіг для самопідписаного Valkey TLS
└── stripe-fake-webhook.ts  # офлайн підписаний вебхук → видати Pro без Stripe/мережі

infra/k8s/local/            # kustomize-база бекенд-сервісів (не застосунок)
├── kind-config.yaml        # kind-кластер + проброс :8080 :8090 :55432 :8025 :9000 :9001
├── 01-postgres.yaml        # Postgres StatefulSet + PVC (NodePort 30432) + pg_trgm init
├── 02-redis.yaml           # Redis (native node-redis, TLS)
...
├── scripts/                # init-скрипти, вмонтовані як ConfigMap (configMapGenerator)
│   ├── minio-bucket-init.sh
│   └── pgadmin-seed-pgpass.sh
└── kustomization.yaml      # база: 5 сервісів + 2 згенеровані ConfigMap (disableNameSuffixHash)

infra/k8s/overlays/local/   # застосунок під тестом = infra/k8s/base + локальні патчі
├── kustomization.yaml
├── secret.local.yaml       # devstash-secrets — локальні «секрети»
├── migrate-job-local.yaml
└── patches/                # NodePort Service, 1 репліка, MinIO-shim sidecar тощо
```

> `devstash-infra local` рендерить базу `infra/k8s/local` й застосовує її **зрізами** (data-сервіси
> ДО migrate-гейта, дашборди — ПІСЛЯ), зберігаючи ті самі readiness-паузи, що й раніше.
> Застосунок (`overlays/local`) застосовується окремо тим самим `apply_overlay_slice`.
...
`prisma/migrations` — лише серверний бандл. Тому `devstash-infra local` запускає
...
STRIPE_WEBHOOK_SECRET=whsec_local_test \
  npx tsx infra/local/stripe-fake-webhook.ts <userId> active
```

**Скрипт:** `infra/local/stripe-fake-webhook.ts`
...
STRIPE_WEBHOOK_SECRET=whsec_local_test \
  npx tsx infra/local/stripe-fake-webhook.ts cltestuser000000000000000 active

# Забрати Pro (скасувати підписку):
STRIPE_WEBHOOK_SECRET=whsec_local_test \
  npx tsx infra/local/stripe-fake-webhook.ts cltestuser000000000000000 canceled
```

`whsec_local_test` — це дефолтне значення `STRIPE_WEBHOOK_SECRET` у `03-app-secret.yaml`,
тож скрипт працює одразу після `devstash-infra local`, без жодного налаштування Stripe. Успіх → `HTTP
...
4. `devstash-infra local deploy` (переприкладає overlay `secret.local.yaml` +
   викочує застосунок), або вручну `kubectl -n devstash rollout restart deploy/devstash-web`.
...
PostgreSQL, до якого Prisma ходить тим самим node-postgres адаптером (`DB_DRIVER=pg`)
...
- [x] Redis (нативний node-redis), S3 (MinIO), email (Mailpit) активні
