...
> [`infra/run/local/run.sh`](../run/local/run.sh) (див. TL;DR). 📚-концепти
> Kubernetes — у [02-kubernetes.md](02-kubernetes.md); тут — реальний запуск наживо.

## Підсумок (TL;DR)

```bash
bash infra/run/local/run.sh          # підняти все (= up)
curl 'http://localhost:8080/api/health?deep=1'   # → {db,redis,s3,email all "ok"}
bash infra/run/local/run.sh deploy   # перезібрати образ і викотити лише застосунок
bash infra/run/local/run.sh status   # стан кластера / подів / health
bash infra/run/local/run.sh info     # вивести всі URL сервісів (застосунок, Postgres, MinIO тощо)
bash infra/run/local/run.sh down     # знести кластер
```
...
```
infra/run/local/            # оркестратор + його сайдкари
├── run.sh                  # up / deploy / status / info / down
...
> `run.sh` рендерить базу `infra/k8s/local` й застосовує її **зрізами** (data-сервіси
...
STRIPE_WEBHOOK_SECRET=whsec_local_test \
  npx tsx infra/run/local/stripe-fake-webhook.ts <userId> active
```

**Скрипт:** `infra/run/local/stripe-fake-webhook.ts`
...
STRIPE_WEBHOOK_SECRET=whsec_local_test \
  npx tsx infra/run/local/stripe-fake-webhook.ts cltestuser000000000000000 active

# Забрати Pro (скасувати підписку):
STRIPE_WEBHOOK_SECRET=whsec_local_test \
  npx tsx infra/run/local/stripe-fake-webhook.ts cltestuser000000000000000 canceled
```
...
4. `bash infra/run/local/run.sh deploy` (переприкладає overlay `secret.local.yaml` +
