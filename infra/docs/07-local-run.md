...
тепер збирається з `infra/k8s/base` через `infra/k8s/local-run/kustomization.yaml`):
...
style, без sidecar. Усі три зміни — лише в `infra/k8s/local-run/` + локальному ConfigMap.
...
├── 02-redis.yaml         # Redis (native node-redis) + RedisInsight UI (NodePort 30801)
