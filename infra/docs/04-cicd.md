...
- **Seed системних `item_types`** — той самий Job після міграцій запускає
  ...
  [`run/local/run.sh`](../run/local/run.sh).
...
> запустити й перевірити з CLI через [`infra/run/gcp/run.sh`](../run/gcp/run.sh):
> ```bash
> bash infra/run/gcp/run.sh secrets   # залити WIF provider + deployer SA + APP_DOMAIN у GitHub (передумова auth@v2)
> bash infra/run/gcp/run.sh deploy    # gh workflow run deploy-gke.yml (build web+migrate → migrate Job → rollout)
> bash infra/run/gcp/run.sh smoke     # дочекатись CI (gh run watch) + health-check /api/health?deep=1
> ```
