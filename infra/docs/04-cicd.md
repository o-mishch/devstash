...
> вказує, яка команда `devstash-infra` запускає й перевіряє pipeline. Передумови WIF
...
- **job `build-deploy`**:
  ...
  - `get-gke-credentials` → **verify control plane reachable** (fail-fast probe of the
    DNS endpoint — перетворює загальний `403 (Forbidden)` GFE на дію, а не на оманливу
    помилку Helm; див. R8 у [09-gcp-audit.md](09-gcp-audit.md)) → render once → apply
    infra → migration gate → server-side apply Deployment → `rollout status --timeout=300s`.
...
- **Seed системних `item_types`** — той самий Job після міграцій запускає
  `SEED_ITEM_TYPES_ONLY=1 DB_DRIVER=pg tsx prisma/seed.ts` (idempotent): без 7
  системних типів застосунок не може створювати items. Той самий патерн, що
  [`devstash-infra local`](../cli/README.md).
...
> запустити й перевірити з CLI через [`devstash-infra` CLI](../cli/README.md):
> ```bash
> devstash-infra gcp secrets   # залити WIF provider + deployer SA + APP_DOMAIN у GitHub (передумова auth@v2)
> devstash-infra gcp deploy    # gh workflow run deploy-gke.yml (build web+migrate → migrate Job → rollout)
> devstash-infra gcp smoke     # дочекатись CI (gh run watch) + health-check /api/health?deep=1
> ```
