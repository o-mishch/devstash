...
└── gcp/                    # GKE: Gateway API, Certificate Manager, Workload Identity, auto-NEG
...
- **`overlays/gcp`** (GKE): **Gateway API** (`gke-l7-global-external-managed`) + глобальна
  статична IP + **Certificate Manager** cert (project-scoped авто-TLS, переживає suspend —
  замість cluster-CRD ManagedCertificate), **HTTPRoute** (маршрут застосунку + HTTP→HTTPS
  redirect замість FrontendConfig), **GCPBackendPolicy** (Cloud Armor / draining / logging) +
  **HealthCheckPolicy** (`/api/health`) замість BackendConfig, NEG створюється контролером
  Gateway автоматично, і ServiceAccount із **Workload Identity**, щоб pod'и зверталися до GCP
  API за ідентичністю (без статичних ключів). Образ вказує на Artifact Registry; CI перевизначає
  тег під кожну збірку.
...
kubectl kustomize infra/k8s/overlays/gcp     # 16 resources (adds SA, Gateway, HTTPRoutes, GCPBackendPolicy, HealthCheckPolicy, ESO)
```
