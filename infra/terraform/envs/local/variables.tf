...
# the kind cluster (modules/kind gates its kind_cluster resource on this via count). devstash-infra local
...
variable "cluster_name" {
  type        = string
  default     = "devstash"
  description = "kind cluster name — must match what devstash-infra / kubectl tooling expects."
}
...
# exists so the devstash-infra CLI could repoint it without editing module code.
