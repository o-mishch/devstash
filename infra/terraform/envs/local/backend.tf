...
# every `run.sh up` to GCP auth and break offline use, and a Kubernetes backend is chicken-
...
#
# Partial backend config: the state path is supplied by infra/run/local/run.sh via
