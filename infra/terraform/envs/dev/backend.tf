...
#
# Chicken-and-egg: this bucket must exist before `tofu init`. The devstash-infra CLI creates the
...
# must use the devstash-infra CLI or pass the explicit bucket with `-backend-config`.
terraform {
  backend "gcs" {
    # Partial backend config: bucket is supplied by the devstash-infra CLI after it
