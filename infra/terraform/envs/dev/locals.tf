locals {
  ...
  # the devstash-infra CLI (via the db_dump_object output) both read this, so the object suspend WRITES
  ...
  # the root module: the db_instance_name output (devstash-infra gcp suspend/resume) and the auto-suspend
