...
from devstash_infra import environment
...
def _make(tf_dir: str, events: list[str]) -> Environment:
    """An Environment wired with real clients; `sleep` records into the shared event log."""

    def _sleep(_seconds: float) -> None:
        events.append("sleep")

    return Environment(_CONFIG, tofu=Tofu(tf_dir), gcloud=Gcloud("proj"), sleep=_sleep)
