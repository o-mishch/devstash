...
class _FakeKubectl:
    ...
    A plain fake satisfying job_gate's `_Kubectl` protocol structurally — no subclassing.
    """
...
def _gate(fake: _FakeKubectl, clock: ManualClock, *, deadline_s: float = 12.0) -> JobGate:
...
def test_complete_returns_immediately_no_diagnostics() -> None:
    ...
    assert clock.monotonic() == 0.0  # resolved on the first poll, never slept
...
def test_failed_aborts_before_deadline_and_dumps() -> None:
    ...
    assert clock.monotonic() == 0.0  # aborted immediately, did not burn the deadline
...
def test_timeout_when_neither_condition_reached() -> None:
    ...
    assert clock.monotonic() == 15.0  # polled 3× (5s gap) until past the 12s deadline
    assert fake.dumped is True
