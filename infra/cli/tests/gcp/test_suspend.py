"""Tests for gcp/suspend.py — the Teardown collaborator + incident fixes #3, #8, #14.
...
from devstash_infra.gcp import suspend
...
# The 5 prevent_destroy addrs — the parity spec (mirrors suspend._PROTECTED_SECRET_ADDRS).
_PROTECTED = (
...
class TestPscStillAttached:
    def test_fix_08_matches_real_gcp_error(self) -> None:
        assert suspend.psc_connections_still_attached(_PSC_ERROR) is True

    def test_fix_08_rejects_unrelated_failure(self) -> None:
        assert suspend.psc_connections_still_attached(_DELPROT_ERROR) is False

    def test_fix_08_rejects_empty(self) -> None:
        assert suspend.psc_connections_still_attached("") is False
...
class TestDownDestroyWithPscRetry:
    def test_fix_08_psc_failure_confirmed_retry_succeeds_second_attempt(
        ...
        monkeypatch.setattr(suspend, "confirm", _yes)
    ...
    def test_fix_08_declining_retry_raises_no_loop(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(suspend, "confirm", _no)
    ...
    def test_fix_08_non_psc_failure_raises_immediately(
        ...
        monkeypatch.setattr(suspend, "confirm", _confirm)
