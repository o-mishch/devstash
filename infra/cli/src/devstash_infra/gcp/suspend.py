"""gcp/suspend.py — the destructive teardown (`down`) family + its incident fixes.
...
is already in gcp/gke.join_fail_fast [fix #11].
...
class Teardown:
    ...
    `sleep` is injected (defaults to `time.sleep`) so the PSC-detach wait [#8] is asserted without
    a real 60s pause.
    """
