"""environment.py — the GCP deploy Environment: config + typed clients + lifecycle methods.
...
class ApplyDeps:
    ...
    path `Reconcile` reads. All land for real with app_gcp.
    """
...
class Environment:
    ...
    `sleep` is injected (defaults to `time.sleep`) so timed behaviors — the #9 IAM cooldown — are
    asserted in tests without a real wait. `Reconcile`/`Gke` collaborators are built on demand from
    this Environment's own config + clients.
    """
    ...
    def apply_exec(self) -> None:
        ...
        self._sleep(IAM_PROPAGATION_COOLDOWN_S)  # [#9] hold the marker PAST apply completion
        self._clear_provisioning()
