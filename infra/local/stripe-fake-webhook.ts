 ...
 *     npx tsx infra/local/stripe-fake-webhook.ts <userId> [active|canceled|past_due]
 *
...
if (!userId) {
  console.error('Usage: npx tsx infra/local/stripe-fake-webhook.ts <userId> [status]')
  process.exit(1)
}
