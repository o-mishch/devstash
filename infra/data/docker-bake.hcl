...
# the --metadata-file, keyed by target name (see the CLI's `ci build-push` step).
#
...
# tier → $0. This bake file is invoked ONLY by CI (the `ci build-push` step); nothing else
# shares the cache, so there is no non-GitHub builder to keep on a registry cache.
#
...
# `run:` step does not, so the workflow runs crazy-max/ghaction-github-runtime just
# before the `ci build-push` step to export them (see .github/workflows/deploy-gke.yml).
#
...
#
# Invoked from repo root by the CLI's `ci build-push` step, which exports the variables
# below. Validate a change without building:
#   IMAGE_URI=a MIGRATE_URI=b GITHUB_SHA=c \
#     docker buildx bake -f infra/data/docker-bake.hcl --print
