...
allowed-tools: Agent, Glob, Grep, Read, Write, Edit, Skill, mcp__context7__resolve-library-id, mcp__context7__query-docs, Bash(git *), Bash(rg *), Bash(grep *), Bash(find *), Bash(ls *), Bash(npm run *), Bash(npx prisma *)
---
...
- `check`: Run a fast read-only housekeeping scan and report findings.
- `run`: Run the same scan, ask which findings to fix, apply approved fixes, then verify.
- `improve`: Run a deep quality audit focused on rule compliance, bugs, security, KISS, DRY, and tests.
...
| `src/app/api/**/*`, `src/actions/**/*`, `src/auth.ts`, `src/lib/auth/**/*`, `src/lib/infra/rate-limit.ts`, `src/lib/db/**/*`, `prisma/**/*` | `security.md` |
...
- Verify edited code before reporting done. For docs-only skill edits, run skill validation instead of app lint/tests.
...
7. If env files or env types changed, compare `.env.example`, `.env`, `.env._production`, and `src/types/env.d.ts`.
8. Run `npm run lint`.
9. Check changed `src/actions/*.ts` and non-DB `src/lib/**/*.ts` for meaningful `*.test.ts` coverage.
...
**Research pass:** If subagent tools are available, spawn a foreground research subagent (single turn) for steps 1–9. The subagent prompt must be self-contained and list all required file paths explicitly — it reads context cold and returns only the rendered audit. The main agent then skips to step 10. This keeps the main context clean for the fix/verify phase. If subagents are unavailable, run steps 1–9 directly in the main thread.

1. Read every `.agents/rules/*` file in full, plus `references/improve-checklist.md` and `references/improve-report.md`.
2. Inventory changed and untracked files. If there is no changeset, say so and stop.
...
11. Apply only approved fixes and verify with `npm run lint`, focused tests or `npm run test:run` when behavior changed, and `npm run build` only when build-specific risk exists.
