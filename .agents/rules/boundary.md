...
- **No new cross-stack imports, but copying source is encouraged.** `src/` must never *import* from `web/` or `backend/`, and vice versa — each workspace has its own dependency graph, build, and deploy pipeline. The decoupling is about the *import edge*, not the *code*: **copying source from `src/` into `web/` (or duplicating logic per-stack) is explicitly allowed and expected**, especially to make the new SPA render pixel-identically to the legacy app. When you port a `src/` component into `web/`, copy it verbatim and adapt only what the stack demands — RSC/async → TanStack Query hooks, `next/link` → TanStack Router, shadcn/Radix → the Base UI kit, the generated-client types — nothing more. Two independent copies that happen to match is the goal; one shared import is the thing to avoid.
