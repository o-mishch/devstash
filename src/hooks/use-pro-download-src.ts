...
export async function getProDownloadUrl(itemId: string, preview = false): Promise<string> {
  const signedUrl = await resolveSignedDownloadUrl(itemId, preview)
  if (signedUrl) return signedUrl
  return getDownloadUrl(itemId, { preview })
}
