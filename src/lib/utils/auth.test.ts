...
describe('oauthEmailIsVerified', () => {
  it('trusts a Google email only when email_verified is asserted', () => {
    expect(oauthEmailIsVerified('google', { email: 'a@b.com', email_verified: true })).toBe(true)
  ...
  it('does NOT trust a Google email when verification is absent or false', () => {
    expect(oauthEmailIsVerified('google', { email: 'a@b.com', email_verified: false })).toBe(false)
    expect(oauthEmailIsVerified('google', { email: 'a@b.com' })).toBe(false)
  })

  it('trusts a GitHub email when the (verified) primary email is present', () => {
    expect(oauthEmailIsVerified('github', { email: 'a@b.com' })).toBe(true)
    expect(oauthEmailIsVerified('github', { email: null })).toBe(false)
  })
  ...
  it('falls back to not-verified for a missing profile or unknown provider', () => {
    ...
    expect(oauthEmailIsVerified('discord', { email: 'a@b.com' })).toBe(false)
  })
})
