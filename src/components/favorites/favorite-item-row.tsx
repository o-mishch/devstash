...
export const FavoriteItemRow = memo(function FavoriteItemRow({ item, onOpen }: FavoriteItemRowProps) {
  const { itemType } = item
  const color = SYSTEM_TYPE_COLORS[itemType.name]

  const style = useMemo(() => ({ '--item-color': color } as CSSProperties), [color])
