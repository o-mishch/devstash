...
import { computeUsage, usageLabel, resolveSkinData, MaybeLink, type DashboardSkinData } from './shared'
...
export async function SpatialSkin(data: DashboardSkinData) {
  ...
  const minis = [
    ...
    ...(isPro
      ? []
      : [{ icon: Zap, value: String(usage.slotsLeft), label: 'Slots left', color: 'var(--primary)', href: undefined as string | undefined }]),
  ]
  ...
  return (
    <div>
      ...
      {/* Pro extends the hero + minis row with a wide Brain Dump cell as the third, widest track. */}
      <div className={`mb-5 grid grid-cols-[1.3fr_1fr] gap-3 sm:gap-5 ${isPro ? 'lg:grid-cols-[1fr_0.9fr_1.6fr]' : 'lg:grid-cols-[1fr_1.4fr]'}`}>
        ...
        <div className={`grid grid-cols-2 gap-4 ${isPro ? '[&>*:last-child]:col-span-2' : ''}`}>
          {minis.map((m) => {
            ...
            return (
              <MaybeLink key={m.label} href={m.href} className={cls}>{inner}</MaybeLink>
            )
          })}
        </div>

        {isPro && <BrainDumpWidget skin="spatial" className="col-span-2 lg:col-span-1" />}
      </div>
