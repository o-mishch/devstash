...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 z-50 bg-black/10 transition-opacity duration-500 ease-out data-ending-style:duration-500 data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs",
        className
      )}
      {...props}
    />
...
function SheetContent({
  ...
  // When this sheet is opened from inside another drawer/dialog (e.g. the collection-create bottom
  // sheet launched from the item drawer on mobile), `elevated` lifts the backdrop + popup to z-[60]
