...
export function ItemDrawerActionBar({ item, isLight, fullItem, onEdit, onDeleted }: ItemDrawerActionBarProps) {
  ...
  const { value: isFavorite, toggle: handleFavoriteToggle } = useOptimisticToggle(
    item.isFavorite,
    (next) => orpcClient.items.toggleFavorite({ id: item.id, isFavorite: next }),
    {
      onSuccess: (next) => {
  ...
  const { value: isPinned, toggle: handlePinToggle } = useOptimisticToggle(
    item.isPinned,
    (next) => orpcClient.items.togglePinned({ id: item.id, isPinned: next }),
    {
      onSuccess: (next) => {
  ...
  async function handleDelete() {
    setIsDeleting(true)
    const { error } = await safe(orpcClient.items.remove({ id: item.id }))
    setIsDeleting(false)
